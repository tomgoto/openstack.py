...
# specify_network controls which network ipaddress to be used. Suitable for
...
def get_host_groups(inventory, network=None, refresh=False):
    (cache_file, cache_expiration_time) = get_cache_settings()
    if is_cache_stale(cache_file, cache_expiration_time, refresh=refresh):
        groups = to_json(get_host_groups_from_cloud(inventory, network=network))
...
def append_hostvars(hostvars, groups, key, server, network, namegroup=False):
    serverID=key
    if network:
      if server['networks'].has_key(network):
        # When --specify_network is used AND its network exists in server.
        ssh_host=server['networks'][network][0]
        key=ssh_host
      else:
        # When --specify_network is used BUT its network does not exist
        ssh_host=server['interface_ip']
        key=ssh_host
    else:
      # When --specify_network is not used.
      ssh_host=server['interface_ip']
    hostvars[serverID] = dict(
...
def get_host_groups_from_cloud(inventory, network):
    ...
    for name, servers in firstpass.items():
        if len(servers) == 1 and use_hostnames:
            append_hostvars(hostvars, groups, name, network, servers[0])
        else:
            server_ids = set()
...
def parse_args():
    ...
    parser.add_argument('--specific_network',
                        action='store',
                        nargs=1,
                        help='Specify private address network for ansible host. If specified network doesnt exist for a instance, returns floatingIP (without --private) or first fixedIP (with --private).')
...
def main():
    ...
    try:
        ...
        if args.list:
          if args.specific_network:
            output = get_host_groups(inventory, network=args.specific_network[0], refresh=args.refresh)
          else:
